...
```{r}
library(tidyverse)
imdb_messy <- read_csv("C:\Desktop\portfolio\data\imdb_5000_messy.csv")
```
...
getwd()
read_csv("initial_steps/imdb_5000_messy.csv")
```
...
```{r}
imdb_messy <- read_csv("initial_steps/imdb_5000_messy.csv")
```
...
read_csv("initial_steps/imdb_5000_messy.csv", n_max = 5)
```
...
```{r}
imdb_temp <- imdb_temp %>% 
 mutate(color = fct_recode(factor(color),
                                 "Black_White" = "B&W",
