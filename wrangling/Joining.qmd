...
havent_voted <- anti_join(contact, voters, by = c("name" = "id"))
head(havent_voted)

# 2. We want contact info for people who HAVE voted
have_voted <- left_join(voters, contact, by = c("id" = "name"))
head(have_voted)

# 3. We want any data available on each person
people <- cross_join(voters, contact)
head(people)

# 4. When possible, we want to add contact info to the voting roster
contact_info <- left_join(voters, contact, by = c("id" = "name"))
head(contact_info)
...
```{r}
median_class_size <- median(courses_combined$enroll, na.rm = TRUE)
head(median_class_size)
```
