...
```{r}
grade_distribution |>
  mutate(grade = fct_collapse(grade, "Satisfactory" = "S", "Audit" = "AU")) |>  # Multiple pieces go into the last 2 blanks
  mutate(grade = fct_relevel(grade, c("A", "A-", "B+", "B", "B-", "C+", "C", "C-", "D+", "D", "D-", "NC", "Satisfactory", "Audit"))) |>
    ggplot(aes(x = grade, y = n)) +
    geom_col()
```
