...
enrollments <- courses_clean %>% 
  ...
  mutate(name = str_replace(name, "Lab", ""))
...
department <- enrollments %>% 
  select(enroll, dept) %>% 
  group_by(dept) %>% 
  summarize(total_enrollment = sum(enroll))

ggplot(department, aes(x = dept, y = total_enrollment, fill = dept)) +
  geom_bar(stat = "identity")
  labs(title = "Total Enrollment by Department", x = "Department", y = "Total Enrollment") +
  theme_minimal() +
